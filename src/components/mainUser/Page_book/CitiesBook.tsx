// src/components/mainUser/Page_book/CitiesBook.tsx
...
const SportsComplexes: React.FC = () => {
      ...
      try {
        ...
        if (Array.isArray(data)) {
          const mapped: Venue[] = data.map((v: any) => ({
            id: v.id,
            name: v.name,
            city: v.city || v.district || "Unknown",
            type: v.type || "Sports Complex",
          }));
          setVenues(mapped);
        } else {
          setVenues([]);
  ...
  const citiesMap = venues.length
    ...
    : fallbackCities.map((c) => ({
        name: c.name,
        venues: c.links.map((l, i) => ({ id: i, name: l, type: "Sports" })),
      }));

  return (
    <section className="bg-gray-50 py-24">
      <div className="flex flex-col items-start px-24">
        <h2 className="text-xl font-bold mb-8 text-gray-800">Top Sports Complexes in Cities</h2>

        <div className="grid gap-10 w-full">
          {citiesMap.map((city, idx) => (
            <div key={idx}>
              <h3 className="text-base font-semibold mb-2 text-gray-700">{city.name}</h3>
              <div className="flex flex-wrap gap-x-2 gap-y-1 text-sm max-h-[40px] overflow-hidden">
                {city.venues.map((venue: any, i: number) => (
                  <React.Fragment key={i}>
                    ...
                    {i !== city.venues.length - 1 && <span className="text-gray-400">·</span>}
                  </React.Fragment>
                ))}
              </div>
            </div>
