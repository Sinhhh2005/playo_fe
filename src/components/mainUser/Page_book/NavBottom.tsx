...
export default function NavBottom({ activeTab, onTabChange }: NavBottomProps) {
	...
	// Fetch counts từ backend
	useEffect(() => {
		const fetchCounts = async () => {
			try {
				const venues = await FieldService.getAllFields();
				// TODO: fetch events, deals nếu có API
				setCounts({
	...
	const tabs = [
		{ id: "venues", label: "Venues", des: "Sports Venues in Bangalore: Discover and Book Nearby Venues", count: counts.venues },
		{ id: "events", label: "Events", des: "Find All events and tournaments in Bangalore", count: counts.events },
		{ id: "deals", label: "Deals", des: "Discover the Best Deals on Gaming Equipment, Game Clothing, Health Instruments and More in Bangalore", count: counts.deals },
	];
		...
		<div className="w-full shadow-md">
			...
			<div className="mt-2 flex gap-6 px-24 py-4">
				{tabs.map((tab) => (
					...
					>
						{tab.label} ({tab.count})
						{activeTab === tab.id && <span className="absolute left-0 bottom-0 h-[2px] w-full bg-green-600" />}
					</button>
				))}
			</div>
		</div>
